...
import { NAVIGATION_ITEMS, XIcon, ChevronDownIcon, ScaleIcon } from '../../constants'; 
            ...
            {item.children.map(child => (
              <li key={child.name} className="py-0.5">
                ...
                >
                  ...
                  <span className="truncate font-normal">{child.name}</span>
                </NavLink>
              </li>
            ))}
          </ul>
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between mb-6 pt-1 px-1">
          <NavLink to="/dashboard" className="flex items-center text-white hover:opacity-90 transition-opacity">
            <ScaleIcon className="w-9 h-9 text-accent-DEFAULT me-2.5" /> {/* Accent for logo icon */}
            <span className="text-3xl font-marhey font-bold text-white dark:text-white">
              عدالة
            </span>
          </NavLink>
          <button onClick={toggleSidebar} className="text-slate-200 hover:text-white md:hidden" aria-label="إغلاق القائمة">
        ...
        <div className="mt-auto pt-4 text-center text-xs text-slate-300 dark:text-slate-400 border-t border-primary-light/30 dark:border-secondary-dark/50">
          &copy; {new Date().getFullYear()} عدالة الحديثة
        </div>
      </aside>
    </>
  );

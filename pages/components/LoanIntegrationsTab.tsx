...
export const LoanIntegrationsTab: React.FC<LoanIntegrationsTabProps> = ({
  ...
  return (
    <div className="space-y-6 text-right" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      <Card className="bg-white" title={lang === 'ar' ? 'مركز الترابط والتكامل البيني الذكي للأنظمة' : 'ERP Tunnels & Modules Synchronization Hub'}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {/* Card 1: HR link */}
          <div className="p-4 border rounded-xl bg-slate-50 relative overflow-hidden flex flex-col justify-between h-32">
            <div>
              <p className="text-[10px] text-slate-400 font-bold">{lang === 'ar' ? 'ربط السجلات العامة' : 'Modules'}</p>
              <h4 className="text-xs font-black text-slate-800 mt-1">{lang === 'ar' ? 'شؤون الموظفين (HR Profiles)' : 'Human Resources Profiles'}</h4>
            </div>
            <div className="flex justify-between items-center mt-3 pt-2 border-t text-[10px] font-bold text-slate-500">
              <span className="flex items-center gap-1"><span className="w-2 h-2 bg-emerald-500 rounded-full"></span> {lang === 'ar' ? 'متصل ومطابق' : 'Live Synced'}</span>
          ...
          <div className="p-4 border rounded-xl bg-slate-50 relative overflow-hidden flex flex-col justify-between h-32">
            <div>
              <p className="text-[10px] text-slate-400 font-bold">{lang === 'ar' ? 'مطابقة الأرصدة والقيود' : 'Modules'}</p>
              <h4 className="text-xs font-black text-slate-800 mt-1">{lang === 'ar' ? 'الإدارة المالية (Ledger)' : 'Corporate Ledgers'}</h4>
            </div>
            <div className="flex justify-between items-center mt-3 pt-2 border-t text-[10px] font-bold text-slate-500">
              <span className="flex items-center gap-1"><span className="w-2 h-2 bg-emerald-500 rounded-full"></span> {lang === 'ar' ? 'متزامن تلقائي' : 'GL Linked'}</span>
          ...
          <div className="p-4 border rounded-xl bg-slate-50 relative overflow-hidden flex flex-col justify-between h-32">
            <div>
              <p className="text-[10px] text-slate-400 font-bold">{lang === 'ar' ? 'الخصم التلقائي بمسيرات الرواتب' : 'Modules'}</p>
              <h4 className="text-xs font-black text-slate-800 mt-1">{lang === 'ar' ? 'الرواتب والأجور (WPS)' : 'Payroll & Wages System'}</h4>
            </div>
            <div className="flex justify-between items-center mt-3 pt-2 border-t text-[10px] font-bold text-slate-500">
          ...
          <div className="p-4 border rounded-xl bg-slate-50 relative overflow-hidden flex flex-col justify-between h-32">
            <div>
              <p className="text-[10px] text-slate-400 font-bold">{lang === 'ar' ? 'تسويات مادة 51 ومستحقات مكافأة نهاية الخدمة' : 'Modules'}</p>
              <h4 className="text-xs font-black text-slate-800 mt-1">{lang === 'ar' ? 'الإنهاءات والمستندات' : 'End of Service Registry'}</h4>
            </div>
            <div className="flex justify-between items-center mt-3 pt-2 border-t text-[10px] font-bold text-slate-500">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <h4 className="text-xs font-black text-slate-500">{lang === 'ar' ? 'منصة العمليات التفصيلية والمزامنات العابرة' : 'Live Sync Transactions Logger Terminal'}</h4>
            <Button 
              size="sm" 
              variant="primary" 

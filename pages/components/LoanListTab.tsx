...
export const LoanListTab: React.FC<LoanListTabProps> = ({
  ...
  return (
    <div className="space-y-6" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="p-6 bg-slate-50 border border-slate-200 rounded-2xl space-y-4">
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="relative flex-grow text-right">
            ...
            <input 
              className="w-full pr-10 pl-4 py-2 text-sm border rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-right"
        ...
        <div className="flex flex-wrap gap-4 items-center justify-between">
          <div className="flex flex-wrap gap-3 items-center">
            <span className="text-xs font-bold text-slate-500">{lang === 'ar' ? 'مرشحات خاصة:' : 'Special Filters:'}</span>
            
            <label className="flex items-center gap-2 cursor-pointer text-xs font-black text-rose-700 bg-rose-50 border border-rose-100 px-3 py-1.5 rounded-lg select-none">
              <input 
                type="checkbox" 
            ...
            <div className="flex items-center gap-2 text-xs font-semibold text-slate-600">
              ...
              <button 
                ...
                className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${salaryRange === 'all' ? 'bg-indigo-600 text-white shadow-xs' : 'bg-slate-200/60 text-slate-600 hover:bg-slate-200'}`}
              >
                {lang === 'ar' ? 'الكل' : 'All'}
              </button>
              <button 
                onClick={() => setSalaryRange('high')}
                className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${salaryRange === 'high' ? 'bg-indigo-600 text-white shadow-xs' : 'bg-slate-200/60 text-slate-600 hover:bg-slate-200'}`}
              >
                {lang === 'ar' ? 'فوق 1,000 د.ك' : '>= 1,000 KWD'}
              </button>
              <button 
                onClick={() => setSalaryRange('low')}
                className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${salaryRange === 'low' ? 'bg-indigo-600 text-white shadow-xs' : 'bg-slate-200/60 text-slate-600 hover:bg-slate-200'}`}
              >
          ...
          <div className="flex gap-2">
            <button 
              ...
              className={`px-3 py-1.5 text-xs font-black rounded-lg border transition-all ${viewMode === 'table' ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white text-slate-600 hover:bg-slate-100'}`}
            >
              {lang === 'ar' ? 'عرض جدول المعاملات' : 'Table View'}
            </button>
            <button 
              onClick={() => setViewMode('card')}
              className={`px-3 py-1.5 text-xs font-black rounded-lg border transition-all ${viewMode === 'card' ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white text-slate-600 hover:bg-slate-100'}`}
            >
      ...
      {/* APPROVAL DECISION DESK (Displays only loans waiting for HR or Finance audit) */}
      {loans.some(l => l.status === LoanStatus.PENDING_APPROVAL || l.status === LoanStatus.UNDER_FINANCIAL_REVIEW) && (
        <div className="p-5 bg-amber-50/50 border border-amber-200 rounded-2xl text-right space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {loans
              ...
              .map(l => {
                ...
                return (
                  <div key={l.id} className="p-4 bg-white border border-amber-200 rounded-xl space-y-3 relative overflow-hidden">
                    <span className="absolute top-2 left-2 text-[8px] px-1.5 py-0.5 font-bold bg-amber-100 text-amber-800 rounded">
                      {l.status}
                    </span>
                    <p className="font-extrabold text-xs text-slate-950">{l.employeeName}</p>
                    <div className="text-[10px] text-slate-500 font-semibold space-y-1">
                      <p>{lang === 'ar' ? 'نوع الطلب:' : 'Type:'} <span className="font-bold text-slate-800">{l.loanType}</span></p>
                      <p>{lang === 'ar' ? 'القيمة المطلوبة:' : 'Requested principal:'} <span className="font-black text-indigo-600">{formatKWD(l.loanAmount)}</span></p>
                      <p>{lang === 'ar' ? 'قسط شهري مقدر:' : 'Monthly repayment:'} <span className="font-bold text-slate-800">{formatKWD(l.monthlyInstallment)} ({l.numberOfInstallments} {lang === 'ar' ? 'أشهر' : 'mon'})</span></p>
                      <p className={`font-black ${isOverCap ? 'text-rose-600' : 'text-emerald-600'}`}>
                        {lang === 'ar' ? 'نسبة الاستقطاع من الراتب الأساسي:' : 'Wage ratio deduction:'} {complianceRatio.toFixed(1)}%
                      </p>
                    </div>
                    {/* Actions buttons */}
                    <div className="flex gap-1.5 pt-2 border-t justify-end">
                      <button 
                        onClick={() => onApproveLoan(l.id, 'reject')}
                        className="px-2 py-1 text-[10px] font-bold bg-rose-50 text-rose-700 rounded border border-rose-200 hover:bg-rose-100"
                      >
                        {lang === 'ar' ? 'رفض الطلب' : 'Reject'}
                      </button>
                      <button 
                        onClick={() => onApproveLoan(l.id, 'audit')}
                        className="px-2 py-1 text-[10px] font-bold bg-slate-100 text-slate-700 rounded border border-slate-300 hover:bg-slate-200"
                      >
                        {lang === 'ar' ? 'مراجعة ثانية' : 'Audit'}
                      </button>
                      <button 
                        onClick={() => onApproveLoan(l.id, 'approve')}
                        className="px-2.5 py-1 text-[10px] font-bold bg-emerald-600 hover:bg-emerald-700 text-white rounded shadow-xs"
                      >
            ...
            <table className="min-w-full divide-y divide-slate-200 text-right">
              ...
              <tbody className="bg-white divide-y divide-slate-100 text-xs font-bold text-slate-700">
                {filteredLoans.map(loan => {
                  ...
                  return (
                    <tr key={loan.id} className="hover:bg-indigo-50/15 transition-colors">
                      <td className="px-6 py-4 font-mono font-bold text-slate-400">{loan.id}</td>
                      <td className="px-6 py-4">
                        <p className="font-black text-slate-900 text-sm">{loan.employeeName}</p>
                        <p className="text-[10px] text-slate-400 mt-1">
                          {emp?.jobTitle} | {lang === 'ar' ? `راتبه بالمسيرة: ${emp?.basicSalary.toFixed(3)} د.ك` : `Basic salary: ${emp?.basicSalary.toFixed(3)} KWD`}
                        </p>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-[10px] bg-indigo-50 text-indigo-700 font-extrabold px-2.5 py-1 rounded-md border border-indigo-100">
                          {loan.loanType}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <p className={`font-black text-sm ${isOverLimit ? 'text-rose-600' : 'text-slate-700'}`}>
                          {formatKWD(loan.monthlyInstallment)}
                        </p>
                        <span className={`text-[9px] font-bold ${isOverLimit ? 'text-rose-500' : 'text-slate-400'}`}>
                      ...
                      <td className="px-6 py-4 font-black text-indigo-950">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredLoans.map(loan => {
            ...
            return (
              <div 
                key={loan.id} 
                className="p-5 bg-white border border-slate-200 rounded-2xl shadow-xs hover:shadow-md transition-all relative overflow-hidden text-right flex flex-col justify-between"
              >
                <div>
                  ...
                  <h4 className="text-md font-black text-slate-900 mb-1 leading-tight">{loan.employeeName}</h4>
                  <p className="text-[10px] text-slate-400 font-bold mb-4">{emp?.jobTitle} | {emp?.department}</p>
                  
                  <div className="space-y-2 border-t border-slate-100 pt-3 text-xs font-bold text-slate-600 leading-relaxed">
                    <div className="flex justify-between">
                      <span>{lang === 'ar' ? 'نوع التمويل والتبويب:' : 'Type:'}</span>
                      <span className="text-slate-800 font-bold">{loan.loanType}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>{lang === 'ar' ? 'قيمة السند الأساسي:' : 'Principal amount:'}</span>
                      <span className="text-indigo-600 font-black">{formatKWD(loan.loanAmount)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>{lang === 'ar' ? 'استقطاع القسط شهرياً:' : 'Monthly repayment:'}</span>
                      <span className="text-slate-800 font-bold">{formatKWD(loan.monthlyInstallment)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>{lang === 'ar' ? 'تاريخ أول قسط:' : 'First installment due:'}</span>
                      <span className="text-slate-400 font-bold">{formatDate(loan.repaymentStartDate)}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-5 space-y-1">
                  ...
                  <div className="flex justify-end gap-1.5 pt-4 mt-3 border-t border-slate-100">

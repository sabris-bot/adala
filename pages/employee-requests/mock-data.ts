...
export const getDefaultDocumentText = (req: EmployeeRequest): string => {
    ...
    switch (req.requestType) {
        case RequestType.END_OF_SERVICE:
            return `مذكرة مخالصة وتصفية مستحقات نهاية الخدمة عمالياً\n(طبقاً لأحكام القانون رقم 6 لسنة 2010 بشأن العمل في القطاع الأهلي بدولة الكويت)\n\nبموجب هذا السند الرسمي الصادر عن مجموعة عدالة للمحاماة والاستشارات القانونية:\nتقر إدارة الموارد البشرية والامتثال بأنه قد تم تصفية مستحقات السيد / ${emp}، الحامل للبطاقة المدنية الكويتية رقم (${req.civilId || ".............."})، والذي كان يشغل منصب (${title}) في قسم (${dept})، اعتباراً من تاريخ انضمامه في ${req.joiningDate || ".............."}.\n\nتفاصيل مدة الخدمة والتصفية العمالية:\n1. سبب إنهاء العلاقة العمالية: (${req.eosReason === 'resignation' ? 'استقالة اختيارية من الموظف' : req.eosReason === 'termination' ? 'إنهاء خدمات من قبل صاحب العمل' : 'تصفية اختيارية / تقاعد'})\n2. مدة الخدمة الإجمالية المعتمدة: (${req.serviceDurationYears || 0}) سنة و (${req.serviceDurationMonths || 0}) شهر عمالي.\n3. نسبة الاستحقاق القانونية طبقاً للمادة (53): (${req.entitledToFullBonus ? "100%" : "خصم نسبي لعدم إتمام السنوات العشر"}).\n\n4. صافي مبلغ مكافأة نهاية الخدمة المستحق للصرف النهائي: (${(req.calculatedIndemnityAmount || 0).toLocaleString()}) د.ك (دينار كويتي فقط لا غير).\n\nباعتماد هذا القرار الإداري والتوقيع عليه من قبل الشركاء والمدير المالي، تبرأ ذمة مجموعة عدالة براءة عمالية تامة ونهائية فور استلام الموظف للمبلغ المالي المسال أعلاه.\n\nالاعتماد والتحقق المالي: ....................\nتوقيع المستشار الشريك: ....................\nالرقم الإداري للمستند المرجعي: ${ref}`;
